...
class BulkMailSendAdmin {
	...
	public function __construct() {
		...
		if ( ! class_exists( 'TT_BulkMailSendUser_List_Table' ) ) {
			require_once dirname( __FILE__ ) . '/class-tt-bulkmailsenduser-list-table.php';
		}
		if ( ! class_exists( 'TT_BulkMailSendOrder_List_Table' ) ) {
			require_once dirname( __FILE__ ) . '/class-tt-bulkmailsendorder-list-table.php';
		}
    ...
    * @param $editor
     ...
     */
    public function my_editor( $editor ) {
        return str_replace('<textarea', '<textarea form="selectmailsend_user_forms"', $editor );
    }
	...
	public function user_selector_page() {
        ...
        add_filter( 'the_editor', [ $this, 'my_editor'] );
        ...
        if ( isset( $_POST['bulk_mail_send_user'] ) && ! empty( $_POST['bulk_mail_send_user'] ) ) {
			...
			if ( check_admin_referer( 'bmsu_send', 'bulk_mail_send_user' ) ) {
				if ( ! empty( $_POST['mails'] ) && ! empty( $_POST['subject'] ) && ! empty( $_POST['body'] ) ) {
					...
					$content = wp_unslash($_POST['body']);
					/* Mail Use HTML-Mails */
					add_filter( 'wp_mail_content_type', array( $this, 'set_html_content_type' ) );
					foreach ( $mails as $key => $value ) {
						$body = str_replace( '%name%', $key, $content );
                        ...
                        /*
                        $file = get_template_directory().'/template-parts/emails/header.php';
                        
                        if( file_exists($file) ) {
                            ob_start();
                            include get_template_directory().'/template-parts/emails/header.php';
                            echo $body;
                            include get_template_directory().'/template-parts/emails/footer.php';
                            $body = ob_get_clean();
                        }
                        */
		...
		<div class="wrap">
			<div style="margin: 5px; padding: 5px;">
				...
				<summary style="cursor: pointer; padding: 10px; border: 1px solid #ddd; background: #f4f4f4; color: #000;"><?php echo esc_html( __( 'Email' ) . ' : ' . __( 'Please select multiple recipients below, and then compose and send an email.', 'bulk-mail-send' ) ); ?></summary>
					...
					<div>
					<?php wp_editor('', 'body', [ 'textarea_name' => 'body'] ); ?>
					</div>
			...
			if ( check_admin_referer( 'bmso_send', 'bulk_mail_send_order' ) ) {
				if ( ! empty( $_POST['mails'] ) && ! empty( $_POST['subject'] ) && ! empty( $_POST['body'] ) ) {
					...
					foreach ( $mails as $key => $value ) {
						...
						$billing = $the_order->data['billing'];
