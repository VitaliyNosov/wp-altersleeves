...
/**
 * Class MC2_Publishing_Functions
 *
...
class MC2_Publishing_Functions {
    ...
    public static function registerNewProductRightsShares( $productRightsShareData ) {
        ...
        foreach( $product_ids as $product_id ) {
            ...
            $author_id = MC2_WP::authorId( $product_id );
            if( !MC2_User_Functions::isAdmin() && ( empty( $author_id ) || $author_id != $productRightsShareData['artistId'] ) ) {
    ...
    public static function checkAndUpdateShareStatus( $share_id, $status, $user_type ) {
        ...
        if( !MC2_User_Functions::isAdmin() && get_current_user_id() != $user_id ) return false;
    ...
    public static function getProductLinkWithImage( $product_id ) {
        if( empty( $idPrinting = MC2_Mtg_Printing_Functions::idForSelection( $product_id ) ) ) return '';
        $printing = new MC2_Mtg_Printing( $idPrinting );

        if( empty( $name_card = $printing->name ) ) return '';

        if( empty( $image = MC2_Alter_Functions::getCombinedImage( $product_id, $idPrinting ) ) ) return '';
