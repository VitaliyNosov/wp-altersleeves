...
use Mythic\Helpers\MC2_Server;
use Mythic\Helpers\MC2_Vars;
use WC_Coupon;

/**
 * Class MC2_Affiliate_Coupons
 *
 * @package Mythic\Coupons
 */
class MC2_Affiliate_Coupon_Functions {
    ...
    public static function registerNewAffiliatePromotion( $coupon_data ) {
        ...
        $wc_register_result = MC2_Coupon::createCoupon( $coupon_data );
    ...
    public static function generatePromotionCode( $wc_coupon_id = 0, $code = '' ) {
        ...
        $code       = !empty( $code ) ? $code : MC2_Vars::generate( 10 );
     ...
     * @return string|null
     */
    public static function getPromotionWcCodeByCode( $mc_code ) {
        if( empty( $promotion_id = self::getPromotionIdByCode( $mc_code ) ) ) return null;

        global $wpdb;
        $table_name = static::$affiliate_coupons_table_name;

        $query = "SELECT promotion_title FROM $table_name WHERE wc_coupon_id = $promotion_id";
    ...
    public static function savePromotionData( $coupon_data, $is_new = false ) {
        ...
        if( !MC2_User_Functions::isAdmin() && $coupon_data['userId'] != get_current_user_id() ) {
            $result['message'] = 'You don\'t have access to update current promotion';
        ...
        $redirectLink = MC2_Vars::stringCleanForUrl( $coupon_data['redirectLink'] );


        $data         = [
            'user_id'                        => $coupon_data['userId'],
            'promotion_title'                => $coupon_data['promotionTitle'],
            'wc_coupon_id'                   => $coupon_data['couponId'],
            'with_discount'                  => $coupon_data['withDiscount'],
        ...
        $data_format = [ '%d', '%s', '%d', '%d', '%d', '%d','%d', '%s', '%d', '%s', '%d', '%d', '%s' ];
    ...
    public static function getRegisterNewAffiliateCouponFields() {
        return [
            ...
            [
                ...
                'options'     => [ 'percent' => 'Percent', 'fixed_cart' => 'Fixed' ],
                'default'     => 'percent',
            ],
            [
    ...
    public static function getAffiliatePromotionDataById( $coupon_id ) {
        ...
        $redirectLink = !empty( $coupon_data['redirect_link'] ) ? MC2_Vars::stringCleanForUrl( $coupon_data['redirect_link'] ) : '';
        $coupon_data  = [
        ...
        $coupon_data['promotionCodes']      = MC2_Template_Parts::get( 'affiliates/parts/as-affiliates-coupons', 'codes',
    ...
    public static function getAffiliatePromotionProductsFromUrl( $url = '' ) : array {
        global $wpdb;
        if( empty( $url ) ) $url = MC2_Server::primaryPath();
    ...
    /**
     * Clear affiliate code cookies after order is created
     *
     * @param $order
     */
    public static function clearAffiliateCodeCookies( $order ) {
            ...
            foreach( $promotion_mc_codes as $promotion_mc_code ) {
                if( !empty( $promotion_id = static::getPromotionIdByCode( $promotion_mc_code ) ) ) {
                    foreach( $applied_coupons as $applied_coupon ) {
                        if( wc_get_coupon_id_by_code( $applied_coupon->get_code() ) == $promotion_id ) {
                            static::markPromotionCodeAsUsed( $promotion_mc_code );
                        }
                    }
                }
            }
    ...
    public static function checkAndApplyAffiliatesCoupons( $with_cookies = true ) {
        ...
        if( !MC2_Environment::primarySite() ) return false;
        ...
        if( !empty( $affiliate_id = $_COOKIE['mc_affiliate_id'] ?? '' ) ) {
            $coupon_code = MC2_Affiliate_Functions::userIdToCoupon( $affiliate_id );
    ...
    public static function generateNoticeMessage( $coupon ) {
        switch( $coupon ) {
            case MC2_Vars::stringContains( $coupon, 'playtowin' ):
            ...
            case MC2_Vars::stringContains( $coupon, 'taaliavess' ):
            ...
            case MC2_Vars::stringContains( $coupon, 'marchesa' ):
            ...
            case MC2_Vars::stringContains( $coupon, 'magicmics' ):
    ...
    public static function applyAffiliateCoupon( $promotion_id ) {
        ...
        if( empty( $coupon_code = MC2_Affiliate_Functions::userIdToCoupon( $affiliate_id ) ) ) return;
    ...
    public static function promotionalProductsInCart( $promotion_id = 0 ) : bool {
        ...
        foreach( $promotion_products as $promotion_product ) {
            if( !empty( MC2_Cart_Functions::productInCart( $promotion_product ) ) ) {
                return true;
            }
        }
        return false;
