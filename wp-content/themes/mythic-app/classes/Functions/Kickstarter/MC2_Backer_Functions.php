 ...
 * Class MC2_Backers
 *
 * @package Mythic\PostTypes
 */
class MC2_Backer_Functions {
    ...
    public function updateBackerData() {
        ...
        if( $query->have_posts() ) : while( $query->have_posts() ) : $query->the_post();
            ...
            $setsRedeemed      = self::array_checker( MC2_WP::meta( 'mc_sets_redeemed', $idBacker ) );
            ...
            if( $user != null ) {
                ...
                if( $setsRedeemed == 1 ) {
                    ...
                    foreach( $orders as $order ) {
                        ...
                        foreach( $order->get_items() as $item_id => $item ) {
                            ...
                            if( MC2_Order_Functions::productCollection( $item ) ) {
                                $bto              = MC2_WP::meta( '_bto_data', $idBacker );
                    ...
                    $sleevesRemaining = MC2_User_Functions::meta( '_download_credits', $user_id );
    ...
    public static function canProgress() {
        ...
        $setsRedeemed = MC2_User_Functions::meta( 'mc_sets_redeemed' );
    ...
    public static function roleBacker() {
        ...
        if( MC2_User_Functions::meta( '_download_credits' ) > 0 ) return true;
    ...
    public static function remainingSingleCredits() : int {
        ...
        $remainingCredits = MC2_User_Functions::meta( '_download_credits', wp_get_current_user()->ID );
    ...
    public function getNormalHeroes() {
        ...
        /** Get all from table */
        echo $query = "SELECT * FROM $tableName WHERE access != 'super_haste' AND ".self::DB_WALL_OF_HEROES_VISIBLE." = 1";
        $results = $wpdb->get_results( $query );
        if( $results == null ) return [];
