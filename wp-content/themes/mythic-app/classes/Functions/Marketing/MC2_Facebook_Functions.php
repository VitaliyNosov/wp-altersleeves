...
use Mythic\Settings\MC2_Data_Settings;
use Mythic\System\MC2_Events;
 ...
 */
class MC2_Facebook_Functions extends MC2_Class {
    
    public function actions() {
        MC2_Action::new( 'mc_facebook_conversion_tracking', [ $this, 'conversionTracking' ], 10, 1 );
    }
    ...
    public static function conversionTracking( $order_id ) {
        ...
        $access_token = MC2_Data_Settings::value( 'facebook_access_token' );
        $pixel_id     = MC2_Data_Settings::value( 'facebook_pixel_id' );
    ...
    public static function pixel() {
        $script = MC2_Data_Settings::value( 'facebook_pixel' );
        if( empty( $script ) || !is_string( $script ) ) return;
        if( !MC2_Vars::stringContains( $script, 'script>' ) ) return;
        echo $script;
    }
