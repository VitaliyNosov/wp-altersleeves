...
$prod_shares_count = MC_Licensing_Functions::getProductRightsSharingCount( 0, $artist_id, $user_id, $product_id );
$pagination_data   = [];
$prod_shares_data  = [];
$data_for_filter   = [];

if( !empty( $prod_shares_count ) ) {
    $prod_shares = MC_Licensing_Functions::getProductRightsSharing( 0, $artist_id, $user_id, $product_id, $per_page, $offset );
    ...
    foreach( $prod_shares as $prod_share_key => $prod_share ) {
        ...
        $prod_shares_data[] = [
            'product' => MC_Licensing_Functions::getProductLinkWithImage( $prod_share['product_id'] )."<span>".$prod_share['product_id']."</span>",
...
$header_data = [ 'Product', 'Artist', 'Status', 'Actions' ];
