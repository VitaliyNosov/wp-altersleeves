...
/**
 * Class MC_Settings_Builder
 *
 * Taken from: https://github.com/boospot/boo-settings-helper/blob/master/class-boo-settings-helper.php
 *
...
class MC_Settings_Builder {
    ...
    public function get_default_settings_url() {
        if( $this->config_menu['submenu'] ) {
            ...
            if( in_array( $options_base_file_name, [
                'options-general.php',
                'edit-comments.php',
                'plugins.php',
                'edit.php',
    ...
    public function get_default_config_menu() {
        return apply_filters( 'boo_settings_filter_default_menu_array', [
    ...
    function callback_checkbox( $args ) {
        ...
        $html .= sprintf( '<input type="checkbox" class="checkbox" id="%1$s[%2$s]" name="%4$s" value="1" %3$s />', $args['section'], $args['id'],
                          checked( $args['value'], '1', false ), $args['name'] );
    ...
    function callback_multicheck( $args ) {
        ...
        foreach( $args['options'] as $key => $label ) {
            ...
            $html    .= sprintf( '<input type="checkbox" class="checkbox" id="%1$s[%2$s][%3$s]" name="%5$s[%3$s]" value="%3$s" %4$s />',
                                 $args['section'], $args['id'], $key, checked( $checked, $key, false ), $args['name'] );
    ...
    function callback_radio( $args ) {
        ...
        if( empty( $value ) ) {
            $value = is_array( $args['default'] ) ? $args['default'] : [];
        }
        ...
        foreach( $args['options'] as $key => $label ) {
            $html .= sprintf( '<label for="%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
            $html .= sprintf( '<input type="radio" class="radio" id="%1$s[%2$s][%3$s]" name="%5$s" value="%3$s" %4$s />', $args['section'],
                              $args['id'], $key, checked( $args['value'], $key, false ), $args['name'] );
            $html .= sprintf( '%1$s</label><br>', $label );
        }
    ...
    function callback_select( $args ) {
        $html = sprintf( '<select class="%1$s-text %5$s" name="%4$s" id="%2$s[%3$s]">', $args['size'], $args['section'], $args['id'], $args['name'],
                         $args['class'] );
    ...
    function callback_file( $args ) {
        ...
        $html = sprintf( '<input type="url" class="%1$s-text wpsa-url" id="%2$s[%3$s]" name="%5$s" value="%4$s"/>', $args['size'], $args['section'],
                         $args['id'], $args['value'], $args['name'] );
        $html .= '<input type="button" class="button mc-browse-button" value="'.$label.'" />';
        $html .= $this->get_field_description( $args );
    ...
    public function callback_media( $args ) {
        ...
        $image_size = ( !empty( $width ) && !empty( $height ) ) ? [ $width, $height ] : 'thumbnail';
        ...
        echo '
                <div class="upload" style="max-width:'.$max_width.';">
                    ...
                    <div>
                        ...
                        <button type="submit" class="mc-image-upload button">'.$text.'</button>
                        <button type="submit" class="mc-image-remove button">&times;</button>
                    </div>
                </div>
            ';
    ...
    function callback_password( $args ) {
        $html = sprintf( '<input type="password" class="%1$s-text" id="%2$s[%3$s]" name="%5$s" value="%4$s"/>', $args['size'], $args['section'],
                         $args['id'], $args['value'], $args['name'] );
        $html .= $this->get_field_description( $args );
    ...
    function callback_color( $args ) {
        $html = sprintf( '<input type="text" class="%1$s-text wp-color-picker-field" data-alpha="true" id="%2$s[%3$s]" name="%6$s" value="%4$s" data-default-color="%5$s" />',
                         $args['size'], $args['section'], $args['id'], $args['value'], $args['default'], $args['name'] );
        $html .= $this->get_field_description( $args );
    ...
    function show_navigation() {
        ...
        foreach( $this->settings_sections as $tab ) {
            ...
            $html         .= sprintf( '<a href="%3$s&tab=%1$s" class="nav-tab %4$s" id="%1$s-tab">%2$s</a>', $tab['id'], $tab['title'],
                                      $settings_page, $active_class );
        }
    ...
    public function script_general() {
        ...
        <script>
            jQuery(document).ready(function( $ ) {
                ...
                // For Files Upload
                $('.mc-browse-button').on('click', function( event ) {
                ...
                // The "Upload" button
                $('.mc-image-upload').click(function() {
                ...
                $('.mc-image-remove').click(function() {
    ...
    public function get_settings_sections() {
        return ( !empty( $this->settings_sections ) ) ? $this->settings_sections : [];
    }
