...
class MC_Url {
    ...
    public static function is( $url = '' ) : bool {
        ...
        if( MC_Vars::stringContains( $url, 'http' ) ) return true;
    ...
    public static function isCartPage() : bool {
        return in_array( MC_Server::primaryPath(), [ 'cart' ] );
    }
    ...
    public static function isCheckoutPage() : bool {
        return in_array( MC_Server::primaryPath(), [ 'checkout' ] );
    }
    ...
    public static function isLoginPage() : bool {
        return in_array( MC_Server::primaryPath(), [ 'wp-login.php', 'wp-register.php', 'login' ] );
    }
    ...
    public static function isDashboard() : bool {
        return in_array( MC_Server::primaryPath(), [ 'dashboard', 'my-account' ] );
    }
    ...
    public static function isRegistrationPage() : bool {
        return in_array( MC_Server::primaryPath(), [ 'registration' ] );
    }
    
    public static function loginUrl() : string {
        return MC_SITE.'/login';
    }
    ...
    public static function matches( $string = '' ) : bool {
        $primary_path = MC_Server::primaryPath();
    ...
    public static function isBrowse() : bool {
        $primary_path = MC_Server::requestUri( false );
        return MC_Vars::stringContains( $primary_path, 'browse' );
    }
    ...
    public static function isCampaign() : bool {
        $primary_path = MC_Server::primaryPath();
        return MC_Vars::stringContains( $primary_path, 'campaign' );
    }
    ...
    public static function contains( $string = '', $url = '' ) : bool {
        ...
        return MC_Vars::stringContains( $url, $string );
    }
    ...
    public static function urlToPath( $string = '', $site_url = '' ) : string {
        ...
        $site_url = !empty( $site_url ) ? $site_url : MC_SITE;
    ...
    public static function current( $clean = true ) : string {
        return ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? "https" : "http" )."://$_SERVER[HTTP_HOST]".'/'.MC_Server::requestUri( $clean );
    }
    ...
    public static function getUrlAndServerLocations( string $unedited = '' ) : array {
        $path = $unedited;
        $path = str_replace( MC_SITE, '', $path );
        $path = str_replace( ABSPATH, '', $path );
        return [
            'path'     => $path,
            'url'      => MC_Url::cleanPath( MC_SITE.$path ),
            'dir'      => MC_Url::cleanPath( ABSPATH.$path ),
            'unedited' => $unedited
        ];
    }
