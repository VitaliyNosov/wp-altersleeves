...
class MC_Backer {
    ...
    public function updateBackerData() {
        ...
        if( $query->have_posts() ) : while( $query->have_posts() ) : $query->the_post();
            ...
            // Personal Info
            $name           = addslashes( MC_WP::meta( 'name', $idBacker ) );
            $addressLine1   = addslashes( MC_WP::meta( 'address_line_1', $idBacker ) );
            $addressLine2   = addslashes( MC_WP::meta( 'address_line_2', $idBacker ) );
            $addressCity    = addslashes( MC_WP::meta( 'address_city', $idBacker ) );
            $addressState   = addslashes( MC_WP::meta( 'address_state', $idBacker ) );
            $addressCountry = addslashes( MC_WP::meta( 'address_country', $idBacker ) );
            $phone          = addslashes( MC_WP::meta( 'phone', $idBacker ) );
            
            /* Access */
            $access  = 'normal';
            $access1 = self::array_checker( MC_WP::meta( 'access1', $idBacker ) );
            $access2 = self::array_checker( MC_WP::meta( 'access2', $idBacker ) );
            if( $access1 == 1 || $access1 ) $access = 'super_haste';
            if( $access2 == 1 || $access2 ) $access = 'haste';
            
            /* Non calculating */
            $set3bl      = MC_WP::meta( 'set3bl', $idBacker );
            $set3bl      = $set3bl != '' ? $set3bl : 0;
            $set4s       = MC_WP::meta( 'set4s', $idBacker );
            $set4s       = $set4s != '' ? $set4s : 0;
            $set5pan     = MC_WP::meta( 'set5pan', $idBacker );
            $set5pan     = $set4s != '' ? $set5pan : 0;
            $set6com     = MC_WP::meta( 'set6com', $idBacker );
            $set6com     = $set6com != '' ? $set6com : 0;
            $setSnapbolt = self::array_checker( MC_WP::meta( 'setsnapbolt', $idBacker ) );
            ...
            foreach( $commissionTypes as $commissionType ) {
                $type = self::array_checker( MC_WP::meta( $commissionType, $idBacker ) );
                if( $type == 1 || $type ) {
            ...
            $setsRedeemed      = self::array_checker( MC_WP::meta( 'mc_sets_redeemed', $idBacker ) );
                ...
                if( $setsRedeemed == 1 ) {
                    ...
                    foreach( $orders as $order ) {
                        ...
                        foreach( $order->get_items() as $item_id => $item ) {
                            ...
                            // If collection
                            if( MC_Woo_Order_Functions::productCollection( $item ) ) {
                                $bto              = MC_WP::meta( '_bto_data', $idBacker, 'user' );
                                ...
                                if( MC_Woo_Order_Functions::productCollectionChild( $item, $order ) ) {
                                    $royalty = 0;
                    ...
                    $sleevesRemaining = MC_WP::meta( '_download_credits', $idUser, 'user' );
    ...
    public static function canProgress() {
        ...
        $setsRedeemed = MC_WP::meta( 'mc_sets_redeemed', wp_get_current_user()->ID, 'user' );
        ...
        if( $setsRedeemed != 1 ) {
            ...
            foreach( $setChecks as $setCheck ) {
                $setCount = MC_WP::meta( $setCheck['key'], $idBacker );
    ...
    public static function roleBacker() {
        ...
        if( MC_WP::meta( '_download_credits', wp_get_current_user()->ID, 'user' ) > 0 ) return true;
        return false;
    }
    ...
    public static function remainingSingleCredits() : int {
        ...
        $remainingCredits = MC_WP::meta( '_download_credits', wp_get_current_user()->ID, 'user' );
