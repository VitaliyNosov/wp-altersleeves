...
/**
 * Class MC_Giftcard_Functions
 *
 * @package Mythic_Core\Functions
 */
class MC_Giftcard_Functions extends MC_Post_Type_Functions {
    
    public static $post_type = 'product';
    
    const DIR_GIFTCARDS = ABSPATH.'/files/giftcards';
    const URI_GIFTCARDS = MC_SITE.'files/giftcards';
    
    public function __construct() {
        if( !is_dir( self::DIR_GIFTCARDS ) ) mkdir( self::DIR_GIFTCARDS, 0755, true );
    }
    ...
    public static function create( $coupon_code = '', $visible_id = 0 ) {
        if( empty( $coupon_code ) ) $coupon_code = MC_Vars::generate( 8, true );
        for( $x = 0; $x <= 100; $x++ ) {
            $exists = get_page_by_title( $coupon_code, OBJECT, 'shop_coupon' );
            if( !empty( $exists ) ) break;
            $coupon_code = MC_Vars::generate( 8, true );
        }
        $args      = [
    ...
    public static function assign_gift_cards_to_order( $order, $force_physical = false ) {
        ...
        foreach( $order_items as $order_item ) {
            ...
            if( !MC_Product_Functions::isGiftCard( $product_id ) ) continue;
        ...
        if( !empty( $assigned_giftcards ) ) update_post_meta( $order_id, 'MC_Gift_Cards', $assigned_giftcards );
        if( $items_in_order == $giftcard_quantity && $giftcard_quantity == $digital_quantity ) MC_Woo_Order_Functions::complete( $order_id );
    }
    
    public static function getGift_CardFromOrder( $order_id ) {
        $giftcards = get_post_meta( $order_id, 'MC_Gift_Cards', true );
    ...
    public static function update_gift_card_balance( $order ) {
        ...
        foreach( $order_items as $order_item ) {
            ...
            if( !MC_Product_Functions::isAlter( $product_id ) ) continue;
