...
use MC_Royalty_Functions;
use MC_Woo_Order;
use MC_WP;

/**
 * Class MC_Transaction_Functions
 *
...
class MC_Transaction_Functions {
    ...
    public static function getTransactionsData( $type = '', $user_id = 0, $order_id = 0, $product_id = 0, $limit = 0,
        ...
        if( !empty( $user_id ) && MC_User_Functions::isAdmin() ) {
            $compare = '=';
            if( is_array( $user_id ) ) {
     ...
     */
    public static function getForAffiliateLedger( $user_id = 0, $limit = false ) {
        global $wpdb;
        $table_name = static::$table_name;
        $query      = "SELECT * FROM $table_name WHERE user_id = $user_id AND type IN('referral_fee', 'withdrawal', 'contracted_fee', 'royalty' )";
        if( $limit ) $query .= " and DATEDIFF(CURDATE(), date) <= 60 and !(site_id = 2 AND date < '2021-08-01')";
    ...
    public static function createTransactionSingle( $key, &$args ) {
        switch( $key ) {
            case 'cost':
                return new MC_Transaction_Business_Costs_Functions( $args );
            case 'contracted_fee':
                return new MC_Transaction_Contracted_Fee_Functions( $args );
            case 'discount':
                return new MC_Transaction_Discount_Functions( $args );
            case 'fee':
                return new MC_Transaction_Fee_Functions( $args );
            case 'misc':
                return new MC_Transaction_Miscellaneous_Functions( $args );
            case 'order':
                return new MC_Transaction_Order_Functions( $args );
            case 'referral_fee':
                return new MC_Transaction_Referral_Fee_Functions( $args );
            case 'refund':
                return new MC_Transaction_Refund_Functions( $args );
            case 'royalty':
                return new MC_Transaction_Royalty_Functions( $args );
            case 'sale':
                return new MC_Transaction_Sale_Functions( $args );
            case 'tax':
                return new MC_Transaction_Tax_Functions( $args );
            case 'withdrawal':
                return new MC_Transaction_Withdrawal_Functions( $args );
        }
    ...
    public static function updateWithdrawals() {
        $withdrawals = MC_Withdrawal_Functions::getAll();
    ...
    public static function createTransactionsFromOrderId( $order_id = 0 ) {
        ...
        $mc_order    = new MC_Woo_Order( $order_id );
        ...
        if( !$promotional ) {
            ...
            if( !empty( $referrers_by_cookie ) ) {
                ...
                foreach( $referrers_by_cookie as $timestamp => $referrer_id ) {
                    ...
                    $referral_amount   = MC_Woo_Order_Functions::referralFee( $order_id, $referrer_id );
        ...
        if( $credits || empty( $payment_method ) ) {
            if( $legacy_backer && !MC_User_Functions::isAdmin( $user_id ) ) {
        ...
        foreach( $order_items as $order_item ) {
            ...
            for( $x = 1; $x <= $quantity; $x++ ) {
                ...
                $sale_args = [
                    ...
                    'user_id'    => MC_WP::authorId( $product_id ),
        ...
        if( !$promotional ) {
            ...
            foreach( $order_items as $order_item ) {
                ...
                if( $site_id == 1 ) {
                    $original_royalties = MC_Royalty_Functions::getByOrderIdAndProductId( $order_id, $product_id );
                    ...
                    for( $x = 1; $x <= $quantity; $x++ ) {
                        $preexisting = self::getRoyaltiesByOrderIdAndProductId( $order_id, $product_id );
                        if( count( $preexisting ) >= $x ) break;
                        $value        = MC_Royalty_Functions::getProductValue( $product_id, $order_id );
                    ...
                    for( $x = 1; $x <= $quantity; $x++ ) {
                        $team_id = MC_Mythic_Frames_Functions::get_team_id_from_product_id( $product_id );
                        foreach( MC_Mythic_Frames_Functions::get_user_ids_in_team( $team_id ) as $user_id ) {
                            $price = $order_item->get_product()->get_price();
                            $value = MC_Mythic_Frames_Functions::get_royalty_percentage_by_user_id( $user_id ) * $price;
                    ...
                    if( $product_id > 265 && $product_id != 898 ) continue;
    
                    $title = get_the_title($product_id);
                    $excerpt = get_the_excerpt($product_id);
                    if( strpos($title, 'set') !== false ) {
                        $excerpt = $excerpt.', Pack Type: Set';
                    }
                    $query = "SELECT ID FROM wp_2_posts WHERE post_excerpt LIKE '%$excerpt%' AND ID != $product_id";
                    $new_product_id = $wpdb->get_var($query );
                    if( empty($new_product_id) ) continue;
                    $wpdb->update('mc_transactions', ['product_id' => $new_product_id ], ['product_id' => $product_id, 'site_id' => 2, 'type' => 'royalty' ]);
                    $wpdb->update('mc_transactions', ['product_id' => $new_product_id ], ['product_id' => $product_id, 'site_id' => 2, 'type' => 'sale' ]);
                }
            }
        }
