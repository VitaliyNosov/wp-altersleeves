...
/**
 * Class MC_Woo_Cart_Item_Functions
 *
...
class MC_Woo_Cart_Item_Functions {
    ...
    public static function ajax( $function = '', $args = [] ) : array {
        switch( $function ) {
            case self::AJAX_ADD_PROMOTIONAL_PRODUCT :
                MC_Affiliate_Coupon::addPromotionalProductToCart( $args );
                break;
            case self::AJAX_REMOVE_BY_PRODUCT_ID :
                $product_id = $args['product_id'] ?? 0;
                if( !empty( $product_id ) ) MC_Woo_Cart_Item_Functions::removeFromCart( $product_id );
                break;
        }
        
        return MC_Woo_Cart_Functions::returnForFrontEnd();
    }
    ...
    public static function getMeta( $cart_item_key, $key = null, $default = null ) {
        ...
        return $data[ $cart_item_key ] ?? $default;
    }
