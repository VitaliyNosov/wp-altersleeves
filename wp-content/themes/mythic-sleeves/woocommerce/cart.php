...
$products = MC_Affiliate_Coupon::getAllPromotionProductIds();
            ...
            if( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item,
                ...
                <tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item',
                    ...
                    <td class="product-name" data-title="<?php esc_attr_e( 'Product', 'woocommerce' ); ?>">
                        ...
                        if( MC_Product_Functions::isAlter( $product_id ) ) {
                            echo MC_Alter_Functions::nameCart( $product_id, $cart_item_key );
                        } else {
                            if( !$product_permalink ) {
                    ...
                    <td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'woocommerce' ); ?>">
                        ...
                        if( ( $_product->is_sold_individually() || in_array( $_product->get_id(),
                                                                             $products ) ) && !MC_User_Functions::isAdmin() ) {
                            $product_quantity = sprintf( $cart_item['quantity'].' <input type="hidden" name="cart[%s][qty]" value="1" />',
                                                         $cart_item_key );
                        } else {
                ...
                <?php if( wc_coupons_enabled() ) { ?>
                    <div class="coupon">
                        <label for="coupon_code"><?php esc_html_e( 'Coupon:', 'woocommerce' ); ?></label> <input type="text" name="coupon_code"
                                                                                                                 ...
                                                                                                                 value=""
                                                                                                                 placeholder="<?php esc_attr_e( 'Coupon code',
                                                                                                                                                'woocommerce' ); ?>" />
                        <button type="submit" class="button" name="apply_coupon"
                                value="<?php esc_attr_e( 'Apply coupon', 'woocommerce' ); ?>"><?php esc_attr_e( 'Apply coupon',
                                                                                                                'woocommerce' ); ?></button>
