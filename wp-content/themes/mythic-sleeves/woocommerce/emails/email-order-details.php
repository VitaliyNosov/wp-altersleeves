...
<div style="margin-bottom: 40px;">
    <table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;"
        ...
        <thead>
        <tr>
            <th class="td" scope="col" style="text-align:<?= esc_attr( $text_align ); ?>;"><?php
            ...
            foreach( $item_totals as $total ) {
                ...
                <tr>
                    <th class="td" scope="row" colspan="2"
                        style="text-align:<?= esc_attr( $text_align ); ?>; <?= ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?>"><?= wp_kses_post( $total['label'] ); ?></th>
                    <td class="td"
                        style="text-align:<?= esc_attr( $text_align ); ?>; <?= ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?>"><?= wp_kses_post( $total['value'] ); ?></td>
                </tr>
                <?php
            }
        }
